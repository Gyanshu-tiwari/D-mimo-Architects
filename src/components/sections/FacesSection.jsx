...
export function FacesSection() {
  const { ref: leftRef, isInView: leftInView } = useInView({ margin: "-80px" });
  const { ref: descRef, isInView: descInView } = useInView({ margin: "-80px" });
  const { ref: photoRef, isInView: photoInView } = useInView({ margin: "-80px" });
      ...
      <Container>
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 xl:gap-16 items-start justify-between">
          ...
          <div className="flex-1 w-full max-w-2xl flex flex-col justify-between">
            <div
              ref={leftRef}
              className={`fade-up mb-6 md:mb-8${leftInView ? " is-visible" : ""}`}
            >
            ...
            <div
              ref={descRef}
              className={`fade-up delay-15 relative pt-6 max-w-xl${descInView ? " is-visible" : ""}`}
            >
          ...
          <div
            ref={photoRef}
            className={`fade-scale delay-25 w-full max-w-80 sm:max-w-95 lg:max-w-105 xl:max-w-115 shrink-0 mx-auto lg:mx-0${photoInView ? " is-visible" : ""}`}
          >
