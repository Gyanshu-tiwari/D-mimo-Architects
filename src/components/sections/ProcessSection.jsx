...
export function ProcessSection() {
  ...
  const { ref: stepsRef, isInView: stepsInView } = useInView({ margin: "-100px" });
    ...
    <Section className="cv-auto bg-white">
      <Container>
        <div ref={headerRef} className="flex flex-col lg:flex-row gap-6 lg:gap-16 mb-8 md:mb-12 items-start justify-between">
          <div className={`fade-up lg:w-1/2${headerInView ? " is-visible" : ""}`}>
          ...
          <div className={`fade-up delay-2 lg:w-1/2 lg:self-end${headerInView ? " is-visible" : ""}`}>
        ...
        <div ref={stepsRef} className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {steps.map((step, i) => (
            <div
              key={step.num}
              className={`fade-up flex flex-col border-t border-gray-200 pt-8${stepsInView ? ` is-visible delay-${i + 1}` : ""}`}
            >
